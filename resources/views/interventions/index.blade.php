                    ...
                    <table class="w-full text-sm">
                        ...
                        @forelse($interventions as $i)
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-gray-700">
                                    {{ $i->date_debut
                                        ? \Carbon\Carbon::parse($i->date_debut)->format('d/m/Y')
                                        : '—' }}
                                </td>
                                ...
                                <td class="px-6 py-4 text-gray-700">
                                    {{ $i->date_fin
                                        ? \Carbon\Carbon::parse($i->date_fin)->format('d/m/Y')
                                        : '—' }}
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="inline-flex items-center gap-3">
                                        ...
                                        <a href="{{ route('interventions.show', $i->id) }}"
                                           class="text-indigo-700 font-semibold hover:underline">
                                            Voir
                                        </a>

                                        @if(auth()->user()->role === 'admin')
                                            <a href="{{ route('interventions.edit', $i->id) }}"
                                               class="text-emerald-700 font-semibold hover:underline">
                                                Modifier
                                            </a>

                                            <form method="POST"
                                                  action="{{ route('interventions.destroy', $i->id) }}"
                                                  class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
